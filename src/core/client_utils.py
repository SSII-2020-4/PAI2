...
class DhEndpoint():
    ...
    For more information: https://medium.com/@sadatnazrul/diffie-hellman-key-exchange-explained-python-8d67c378701c
    Server = DH_Endpoint()
    Client = DH_Endpoint()
    """
    ...
    def __init__(self, group=14):
        ...
        Arguments:
            group {int} -- Choose Internet Key Exchange (IKE) (default: {14})
            For more information: https://tools.ietf.org/html/rfc3526#section-6
        """
        if group in self.__primes:
            self.p = self.__primes[group]["prime"]
            self.g = self.__primes[group]["generator"]
        else:
            raise Exception("Group not supported")
        self.__private_key = int(binascii.hexlify(os.urandom(32)), base=16)
        self.full_key = None

    def generate_partial_key(self):
        """
        Generate partial key: Client ---- Server
        key_partial = p ^ key_priv % g
    ...
    def generate_full_key(self, partial_key_r):
        """
        Generate full key: Client ---- Server
        key_full = key_partial ^ key_priv % p

        Arguments:
            partial_key_r {int} -- Partial key

        Returns:
            int -- Full key
        """
        self.full_key = pow(partial_key_r, self.__private_key, self.p)
